...
export class RegionFocusSwitcher extends Disposable {
  ...
  public getCurrentPanelElement(): HTMLElement | null {
    const region = this._state.get().region;
    return region?.type === "panel" ? getPanelElement(region.id) : null;
  }

  public togglefocusSectionHeader() {
    const current = this._state.get().region;
    const gristDoc = this._getGristDoc();
    if (!gristDoc) {
      return;
    }
    if (current?.type === "panel" && current.id.startsWith("section-header-")) {
      this._focusRegion({
        type: "section",
        id: gristDoc.viewModel.activeSectionId(),
      }, { initiator: { type: "jump" } });
    }
    if (current?.type === "section") {
      this._focusRegion({
        type: "panel",
        id: `section-header-${gristDoc.viewModel.activeSectionId()}`,
      }, { initiator: { type: "jump" } });
    }
  }
